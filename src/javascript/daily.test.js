...
test("check sortingFunctions", () => {
	...
	expect(sortOriginReverse[0].origin).toBe("Oregon");
	expect(sortOriginReverse[7].origin).toBe("Brazil");
	expect(sortOriginReverse[8].origin).toBe("BC");

	const sortAlpha = functions.sortAlpha(myArray);
	expect(sortAlpha[0].str).toBe("apples");
	expect(sortAlpha[1].str).toBe("avocados");
	expect(sortAlpha[8].str).toBe("pineapple");
...
test("for each: email builder for company", () => {
	...
	expect(staffEmailForEach[0]).toEqual("jane.smith@evolveu.ca");
	expect(staffEmailForEach[3]).toEqual("olivia.notly@evolveu.ca");
	expect(staffEmailForEach[6]).toEqual("benjamin.amis@evolveu.ca");
});

test("map: email builder for company", () => {
	const staffEmailMap = functions.loopStaffMap(data.staff);

	expect(staffEmailMap[0]).toEqual("jane.smith@evolveu.ca");
	expect(staffEmailMap[3]).toEqual("olivia.notly@evolveu.ca");
	expect(staffEmailMap[6]).toEqual("benjamin.amis@evolveu.ca");
});
...
test("for in: email builder for company", () => {
	...
	expect(staffEmailIn[3]).toEqual("olivia.notly@evolveu.ca");
	expect(staffEmailIn[6]).toEqual("benjamin.amis@evolveu.ca");
});

test("for of: email builder for company", () => {
	const staffEmailOf = functions.loopStaffOf(data.staff);
	expect(staffEmailOf[0]).toEqual("jane.smith@evolveu.ca");
	expect(staffEmailOf[3]).toEqual("olivia.notly@evolveu.ca");
	expect(staffEmailOf[6]).toEqual("benjamin.amis@evolveu.ca");
});
...
test("check filter users over 50", () => {
	expect(functions.filter()).toEqual([
