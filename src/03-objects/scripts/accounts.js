...
class AccountController {
    ...
    getAccountIndex (accountName) {
        let accounObject;
        this.userAccounts.forEach(key => {
            if (key.accountName === accountName) {accounObject = key;};
        }); 
    ...
    operationControl (transaction, amount, accountIndex) {
        ...
        if (transaction === 'deposit') {
            ...
            this.message = 
                `$${amount} have been deposited to your ${accountName} account.`;
            return this.userAccounts[accountIndex].balance;
        } else{
            let withdraw = this.userAccounts[accountIndex].withdraw(amount);
            if (withdraw === false){
                this.message = 'There is not enough fund in the selected account.';
                return;
            }else{
                this.balance = withdraw;
                this.message=
                    `$${amount} have been withdrawed from your ${accountName} account.`;
                return this.userAccounts[accountIndex].balance;
            };   
        };
    ...
    maxBalance () {
        this.userAccounts.sort((account1, account2) => 
