...
def num_size(a):
    if a <= 0:
        return f'{a} is a negative number'
    elif a <= 50:
        return f'{a} is a small number'
    elif a <= 100:
        return f'{a} is a medium number'
    else:
        return f'{a} is a large number'
...
def email (f_name, l_name):
    return f'{f_name}.{l_name}@evolveu.ca'.lower()