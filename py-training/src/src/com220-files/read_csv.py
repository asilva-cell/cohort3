...
def read_csv_census(file_name):
    ...
    with open(file_name, 'r') as file:
        ...
        for row in reader:
            people_count_by_zone_type[row['CLASS']
                                      ]['population'] += int(row['RES_CNT'])
            people_count_by_zone_type[row['CLASS']]['count'] += 1
            people_count_by_sector[row['SECTOR']
                                   ]['population'] += int(row['RES_CNT'])
...
def create_census_report(zone_data, sector_data):
    ...
    string += "Zone Type".ljust(20) + \
        "Population".ljust(20) + "Lines".ljust(20) + "\n"
    string += "-".center(60, "-") + "\n"
    for zone_type, data in zone_data.items():
        string += f"{zone_type}".ljust(20) + f"{data['population']}".ljust(
            20) + f"{data['count']}".ljust(20) + "\n"

    string += "\n"*2

    string += " POPULATION BY SECTOR ".center(60, "-") + "\n"
    string += "Sector".ljust(20) + "Population".ljust(20) + \
        "Lines".ljust(20) + "\n"
    string += "-".center(60, "-") + "\n"
    for sector, data in sector_data.items():
        string += f"{sector}".title().ljust(20) + \
            f"{data['population']}".ljust(
                20) + f"{data['count']}".ljust(20) + "\n"
